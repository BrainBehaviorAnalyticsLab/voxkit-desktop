...
class W2TGEngine(AlignmentEngine):
    def run_alignment(self, audio_root, output_root, settings: AlignmentSettings):
        models = list_models("W2TG", True)

        if (
            settings.get("aligner_model") is None
            or settings.get("aligner_model") not in models.keys()
        ):
            print(
                f"Warning: Aligner model '{settings.get('aligner_model')}' not found. Using default settings."
            )
