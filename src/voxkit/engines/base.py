...
class AlignmentEngine(ABC):
    ...
    def transcribe(self, dataset_id: str) -> None:
        """
        Transcribe audio files in a dataset to produce label files.

        Implementations should process the dataset identified by
        ``dataset_id``, transcribing each audio file and writing
        the result to an adjacent ``.lab`` file.
    ...
    def _validate_transcribe_settings(self, settings: dict) -> bool:
        """
        Validate transcription settings for the engine.

        Args:
            settings: Dictionary of transcription settings loaded from JSON.
    ...
    def get_settings(self, tool_type: ToolType) -> dict:
        ...
        # Validate settings
        validators = {
            "train": self._validate_train_settings,
            "align": self._validate_align_settings,
            "transcribe": self._validate_transcribe_settings,
        }
        is_valid = validators[tool_type](settings)
